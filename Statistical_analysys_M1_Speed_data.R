...
pred_df_south <- data.frame(day_numeric = 1:7, 
                            predicted_speed_south = predict(poly_mod_south, 
                                                            newdata = data.frame(day_numeric = 1:7)))
View(pred_df_south)
predicted_speed_south <- pred_df_south$predicted_speed_south

error_s <- M1_avg_speed_num$mean_speed_south - predicted_speed_south
pred_df_south <- data.frame(day_numeric = 1:7,
                            actual_mean_speed = mean_speed_south, 
                            predicted_mean_speed = predicted_speed_south,
                            error = error_s)
View(pred_df_south)

spearm_cor_north<-cor(rank(M1_avg_speed_num$day_numeric), rank(M1_avg_speed_num$mean_speed_north), method = "spearman")
spearm_cor_south<-cor(rank(M1_avg_speed_num$day_numeric), rank(M1_avg_speed_num$mean_speed_south), method = "spearman")
spearm_cor_north
spearm_cor_south
...
correlation_n<-cor(road_conditions_df$Speed_N, road_conditions_df$Road_conditions_N)
correlation_n
correlation_s<-cor(road_conditions_df$Speed_S, road_conditions_df$Road_conditions_S)
correlation_s
